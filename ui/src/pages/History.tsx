...
export default function History() {
  ...
  const handleStopRun = async (e: React.MouseEvent, runId: string) => {
    ...
    try {
      await stopRun(runId);
      toast("Simulation stopped and removed from history", "info");
      refresh();
    } catch (err: any) {
  ...
  const SortHeader = ({ label, field }: { label: string; field: SortKey }) => (
    <th
      className="px-6 py-4 cursor-pointer hover:bg-slate-100/50 transition-colors select-none text-[10px] font-black tracking-widest text-slate-400 uppercase text-left whitespace-nowrap"
      onClick={() => toggleSort(field)}
    >
      <span className="flex items-center gap-1.5">
        {label}
        {sortKey === field && <ArrowUpDown size={11} className="text-[#141E52]" />}
      </span>
    </th>
  );

  return (
    <div className="min-h-screen bg-[#f8fafc] p-4 md:p-8 text-left" dir="ltr">
      <div className="max-w-[1400px] mx-auto space-y-6">
        
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-slate-200 pb-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-[#141E52] tracking-tight uppercase">Run History</h1>
            <p className="text-[11px] text-slate-400 mt-1 font-bold">Review system execution logs</p>
          </div>
          <div className="flex items-center justify-between sm:justify-end gap-3 w-full sm:w-auto">
            {/* כפתור רענון בצבע כחול מורשה #274D96 */}
            <button onClick={refresh} disabled={loading} className="cursor-pointer text-[10px] font-black text-[#274D96] uppercase tracking-widest disabled:opacity-50 hover:opacity-80 transition-opacity">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center bg-white p-4 rounded-[6px] border border-slate-100 shadow-sm">
          ...
          <select
            ...
            className="w-full sm:w-auto bg-white border border-slate-200 rounded-[6px] px-3 py-2 text-xs font-bold outline-none min-w-[200px] focus:border-[#274D96]"
          >
          ...
          <select
            ...
            className="w-full sm:w-auto bg-white border border-slate-200 rounded-[6px] px-3 py-2 text-xs font-bold outline-none min-w-[160px] focus:border-[#274D96]"
          >
            ...
            <table className="w-full text-sm min-w-[900px]">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100 text-left">
                  ...
                  <th className="px-6 py-4 text-[10px] font-black tracking-widest text-slate-400 uppercase text-left whitespace-nowrap">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {sortedRuns.map((run) => {
                  ...
                  // התאמת צבעים חכמה ומורשת לפי החוקים הסגורים שלך
                  let statusColor = "text-slate-700";
                  if (active) statusColor = "text-[#274D96] font-black"; // כחול מורשה לתהליך
                  else if (run.status === "Passed") statusColor = "text-[#59660F] font-bold"; // ירוק זית מורשה להצלחה
                  else if (run.status === "Failed") statusColor = "text-[#D65527] font-bold"; // אדום/כתום מורשה לכישלון
                  ...
                  return (
                    ...
                    >
                      <td className="px-6 py-4 font-bold text-[#141E52] text-xs">
                        {run.simulation_name}
                      </td>
                      ...
                      {/* סימון שגיאות באדום/כתום המורשה #D65527 */}
                      <td className={`px-6 py-4 tabular-nums text-xs font-bold ${(run.results?.errors ?? 0) > 0 ? 'text-[#D65527]' : 'text-black'}`}>
                      ...
                      <td className="px-6 py-4 text-xs font-medium whitespace-nowrap text-slate-600">
                        {active ? (
                          <span className="text-[#274D96] font-bold animate-pulse text-[11px]">Running...</span>
                        ) : run.end_time ? (
                      ...
                      <td className="px-6 py-4 text-xs whitespace-nowrap">
                        {active && (
                          <button
                            ...
                            className="cursor-pointer flex items-center gap-1.5 px-3 py-1 bg-rose-50 text-[#D65527] border border-rose-100 rounded-[4px] font-black text-[9px] tracking-wide hover:bg-[#D65527] hover:text-white transition-all uppercase"
                          >
