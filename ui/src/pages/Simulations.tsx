...
import { Plus, Play, Trash2, Pencil, Upload, Activity, Search } from 'lucide-react';
...
const Simulations: React.FC = () => {
  ...
  if (loading) return <div className="p-8 text-center text-slate-500 font-medium">טוען סימולציות...</div>;

  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto" dir="ltr">
      {/* Top Header Row */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <div className="flex gap-3 w-full sm:w-auto">
          ...
          <button 
            ...
            className="cursor-pointer flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-[#274D96] text-white rounded-lg hover:bg-[#37A8D8] transition-colors text-xs md:text-sm lg:text-base font-medium whitespace-nowrap shadow-sm"
          >
            <Plus size={18} />
            New Simulation
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredSims.map((s) => (
          <div 
            ...
            className="group bg-white border border-slate-200 rounded-[24px] p-5 shadow-sm hover:shadow-xl hover:shadow-[#37A8D8]/10 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between min-w-0"
          >
            <div>
              {/* Card Header Actions */}
              <div className="flex justify-between items-start mb-4">
                {/* אייקון ה-Activity שמגיב לריחופים בצבע תכלת מורשה */}
                <div className="p-3 bg-[#f0eeff] rounded-xl text-[#5c4cf4] group-hover:bg-[#37A8D8] group-hover:text-white transition-all duration-300 shrink-0 transform group-hover:scale-110 shadow-sm">
                  <Activity size={24} />
                </div>
                <button 
                  onClick={(e) => {
              ...
              {/* כותרת השם שמקבלת גוון כחול עמוק בראנד בריחוף */}
              <h3 className="text-base sm:text-lg md:text-xl lg:text-2xl font-medium text-slate-800 mb-2 leading-snug break-words group-hover:text-[#274D96] transition-colors duration-300">
                {s.simulation_name}
              </h3>
              
              <p className="text-[10px] sm:text-xs md:text-sm font-normal text-slate-400 mb-4 break-all select-all bg-slate-50 p-2 rounded-xl border border-slate-100 group-hover:bg-slate-100/50 transition-colors">
              ...
              <div className="flex items-center gap-2 mb-6 border-t border-slate-50 pt-4"></div>
            </div>
            ...
            <div className="flex gap-2 mt-auto">
              ...
              <button 
                ...
                className="cursor-pointer flex-1 flex items-center justify-center gap-1.5 bg-[#274D96] text-white py-2.5 rounded-lg hover:bg-[#37A8D8] font-medium text-xs sm:text-sm md:text-base transition-colors shadow-sm"
              >
